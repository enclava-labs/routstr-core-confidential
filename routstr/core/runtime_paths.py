...
CAP_APP_DATA_DIR = Path("/app/data")
...
def resolve_data_dir(
    ...
    *,
    ...
    cap_app_data_dir: Path = CAP_APP_DATA_DIR,
) -> Path:
    ...
    if cap_app_data_dir.is_dir():
        return cap_app_data_dir
...
def resolve_log_dir(
    ...
    *,
    ...
    CAP deployments keep application state durable under the declared
    /app/data bind mount, but request logging is synchronous. Keep default logs
    off the encrypted state mount so state I/O stalls cannot block every app
    route before handlers run. Operators can still opt into durable logs with
    ROUTSTR_LOG_DIR.
    """
    env = os.environ if environ is None else environ
    ...
    if (
        cap_app_data_dir.is_dir()
        or cap_state_data_dir.is_dir()
        or _env_value(env, "ROUTSTR_DATA_DIR")
    ):
